...
/**
 * Sealed event hierarchy - 19 record 子类型，pattern matching switch 保证编译时穷尽。
 *
 ...
 *
 * <p>D-04: record 字段锁定见各 CONTEXT.md 表格。{@code PlanEvent.description} 与
 * {@code StepCompleteEvent.status} 为占位字段，Phase 6/7 实现模式时再补全。
 * {@code TotNodeEvent} / {@code TotPruneEvent} 为 Phase 8 新增（树节点语义）。
 * {@code ReflexionAttemptEvent} / {@code ReflexionEvaluateEvent} / {@code ReflexionReflectEvent}
 * 为 Phase 9 新增（Reflexion 尝试-评估-反思循环语义）。
 * {@code RolePmEvent} / {@code RoleDevEvent} / {@code RoleTesterEvent}
 * 为 Phase 10 新增（Role-playing PM/Dev/Tester 角色对话语义）。
 * {@code ReasoningDeltaEvent} / {@code RoleSpeechDeltaEvent} 为流式改造新增
 * （临时态增量事件，分别由完整 ReasoningEvent / Role*Event 收口，
